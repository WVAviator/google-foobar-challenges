...
def getSequenceIdentity(number):
    mod = number % 4
    if mod == 0:
        return number
    elif mod == 1:
...
def getXorOfSequence(start, end):
    return getSequenceIdentity(start - 1) ^ getSequenceIdentity(end)

def solution(start, length):
    result = 0
    for i in range(0, length):
        result ^= getXorOfSequence(start + length * i, start + length * i + (length - 1 - i)) 
    return result
...
equals("Test 1", 2, lambda: solution(0, 3))
equals("Test 2", 14, lambda: solution(17, 4))
equals("Test 3", 41861120, lambda: solution(10000000, 5000))
equals("XORSequenceTest1", 12, lambda: getXorOfSequence(7, 10))
equals("XORSequenceTest2", 8, lambda: getXorOfSequence(101, 108))